#!/usr/bin/env npx tsx
// ============================================
// BUILD SCRIPT FOR TREEZ INGEST LAMBDA
// Bundles src/lambda/treez-ingest-handler.ts with esbuild,
...
const DIST = join(ROOT, 'dist', 'lambda-treez-ingest');
const ENTRY = join(ROOT, 'src', 'lambda', 'treez-ingest-handler.ts');
const OUTPUT = join(DIST, 'index.js');
const ZIP_PATH = join(DIST, 'treez-ingest-handler.zip');

async function main() {
  console.log('=== Building Treez Ingest Lambda ===\n');
  ...
  await build({
    ...
    // csv-parse is NOT externalized — it gets bundled into the output
    external: [
      '@prisma/client',
      '@aws-sdk/*',
    ],
  ...
  // Copy @prisma/client package for the require chain
  ...
  // Copy Prisma client runtime directory
